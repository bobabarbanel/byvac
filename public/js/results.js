$(function () {
    ...
    $("#bar").css("max-width", $('table').css('width') + "px").css("width", 0.1 + "%")
    ...
    async function refreshValues() {
        ...
        // Keep these separate in case sessionToken needs resetting
        // if(rcount++ % 3 == 1) sessionToken += "bug";
        let value;
        value = await axios.get('/refreshCOMPLETED');
        COMPLETED = value.data.COMPLETED;
        value  =  await axios.get('/refreshOPEN');
        ...
        const transitionColor = '#AED6F1';
        $("#completed").css("background", transitionColor).text(COMPLETED);//

        $("#open").css('background', transitionColor).text(OPEN);
        const total = OPEN + COMPLETED;
        $("#total").css('background', transitionColor).text(total);

        setTimeout(() => {
            $("#total, #completed").css("background", "white");
            $("#open").css('background', 'pink');
        }, 500)
    }
